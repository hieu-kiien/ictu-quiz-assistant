...
export default {
  async fetch(request, env) {
    ...
    const corsHeaders = {
      ...
      "Access-Control-Allow-Headers": "Content-Type, Authorization, X-API-Key, X-App-Secret",
    };
    ...
    // 1. API: Giai de thuc te & tiem dap an (Strict App Secret & API Key Guard)
    if (url.pathname === "/api/solve" && request.method === "POST") {
      try {
        const body = await request.json();
        
        // AUTHENTICATION: Check App Secret Token
        const expectedSecret = env.APP_SECRET || DEFAULT_APP_SECRET;
        const incomingSecret = request.headers.get("X-App-Secret") || body.app_secret || "";
        
        if (incomingSecret !== expectedSecret) {
          return new Response(JSON.stringify({ error: "403 Forbidden: Sai hoặc thiếu mã App Secret Token bảo vệ Worker." }), {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }

        const apiKey = (body.api_key || request.headers.get("X-API-Key") || "").trim();
        const modelName = body.model || "gemini-3.6-flash";
        const quizData = body.payload;

        if (!apiKey) {
          return new Response(JSON.stringify({ error: "401 Unauthorized: Vui lòng cung cấp Google Gemini API Key." }), {
            status: 401,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }
        ...
        if (!quizData) {
          return new Response(JSON.stringify({ error: "400 Bad Request: Thiếu dữ liệu đề thi (payload rỗng)." }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }
    ...
    // 2. API: Chay thu nghiem giai de mau (Test Run with Auth)
    if (url.pathname === "/api/test-solve" && request.method === "POST") {
      try {
        ...
        if (incomingSecret !== expectedSecret) {
          return new Response(JSON.stringify({ status: "error", message: "403 Forbidden: Sai mã bảo vệ Server (App Secret Token)." }), {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }
        ...
        if (!apiKey) {
          return new Response(JSON.stringify({ status: "error", message: "401 Unauthorized: Vui lòng dán mã Google Gemini API Key trước khi thử nghiệm!" }), {
            status: 401,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }
...
// Core AI Solver with Intelligent Error Classification & Strict Fallback
async function solveAndInject(data, apiKey, modelName) {
  ...
  for (const m of modelsToTry) {
    try {
      ...
      if (msg.includes("400") || msg.includes("api_key_invalid") || msg.includes("invalid api key") || msg.includes("api key not valid")) {
        throw err;
      }

      if (msg.includes("high demand") || msg.includes("overloaded") || msg.includes("exhausted") || msg.includes("429") || msg.includes("503") || msg.includes("500")) {
        continue;
      }
...
4. Chỉ trả về JSON thuần, KHÔNG kèm markdown hay giải thích thừa.`;
  ...
  if (!response.ok) {
    ...
    if (errBody && errBody.error && errBody.error.message) {
      errMsg = `${errBody.error.message} (${response.status})`;
    }
            ...
            if (it && typeof it === "object") {
              for (const tk of ["text", "title", "content", "label"]) {
                if (typeof it[tk] === "string") {
                  ...
                  if (lower === target || lower === matchItem) {
