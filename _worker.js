...
// Core AI Solver & Invisible Hint Injection Engine with Auto-Failover
async function solveAndInject(data, apiKey, modelName) {
  ...
  for (const m of modelsToTry) {
    try {
      ...
      lastError = err;
      // Neu loi la high demand hoac 429/503 -> Thu model tiep theo
      if (err.message && (err.message.includes("high demand") || err.message.includes("429") || err.message.includes("503"))) {
        continue;
      } else {
        throw err;
      }
...
Nhiệm vụ: Giải chính xác danh sách câu hỏi và trả về duy nhất một JSON:
{
  "results": [
    ...
    { "index": 3, "type": "choice", "correct_option_indices": [0] }
  ]
}
Quy tắc:
1. correct_option_indices: mảng index phương án đúng (bắt đầu từ 0).
2. hint_text: từ hoặc cụm từ cần điền.
