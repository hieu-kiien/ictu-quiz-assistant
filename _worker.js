// ICTU Quiz Assistant - Cloudflare All-In-One Edge Worker (Comprehensive Fix)
...
// Core AI Solver & Invisible Hint Injection Engine (Multi-Format & Anti-Duplicate)
...
async function callGeminiModel(data, apiKey, modelName) {
  ...
  const systemPrompt = `Bạn là chuyên gia giải đề thi đại học trên hệ thống LMS/Moodle (Đặc biệt là các môn CNTT, Triết học, Pháp luật, Tiếng Anh).
Nhiệm vụ: Giải chính xác toàn bộ danh sách câu hỏi dưới đây và trả về DUY NHẤT một JSON theo cấu trúc sau:
{
  "results": [
    ...
    { "index": 3, "type": "match", "pairs": [{"target": "Thủ đô", "match": "Hà Nội"}] }
  ]
}
Quy tắc:
1. correct_option_indices: mảng chứa các chỉ số (index từ 0) của phương án đúng. Nếu chọn 1 đáp án thì có 1 phần tử [0], nếu chọn nhiều đáp án thì chứa tất cả [0, 2].
2. hint_text: từ hoặc cụm từ chính xác cần điền vào ô trống.
3. Chỉ trả về JSON thuần, không giải thích thừa.`;
  ...
  const questions = data.questions || data.question || (Array.isArray(data) ? data : []);
  ...
  for (const item of results) {
    ...
    if (idx !== undefined && idx < questions.length) {
      ...
      // Tim mang chua cac phuong an lua chon
      let optArray = null;
      let optKey = "";
      ...
      // 2. Dien tu vao o trong -> Chen goi y vao cuoi cau hoi (Chong tiem trung lap)
      else if (qType === "fill") {
      ...
      // 3. Keo tha / Ghep cap -> Danh so cap [1], [2]...
        ...
        pairs.forEach((pair, pIdx) => {
          ...
          if (Array.isArray(q.items || q.options)) {
            const arr = q.items || q.options;
            arr.forEach(it => {
              if (it && typeof it === "object") {
                for (const tk of ["text", "title", "content", "label"]) {
                  if (typeof it[tk] === "string" && (it[tk].includes(target) || it[tk].includes(matchItem))) {
